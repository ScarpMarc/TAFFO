...
bool createPow(FloatToFixed *ref, llvm::Function *newfs, llvm::Function *oldf)
{
    ...
    flttofix::FixedPointType fxpret1;
    flttofix::FixedPointType fxpret2;
    ...
    TaffoMath::getFixedFromArg(ref, oldf, fxpret1, 0, foundArg1);
    TaffoMath::getFixedFromArg(ref, oldf, fxpret2, 1, foundArg2);
    ...
    ExpFunType funcType = ExpFunType::Exp;

    if (!fxpret.scalarIsSigned()) {
        LLVM_DEBUG(dbgs() << "Argument is unsigned: shifting it to the right by 1 bit\n");
        fxpret.scalarFracBitsAmt() = fxpret.scalarFracBitsAmt() - 1;
        fxpret.scalarIsSigned() = true;
    }

    BasicBlock *second_op = BasicBlock::Create(cont, "second_op", newfs);
    BasicBlock *third_op = BasicBlock::Create(cont, "third_op", newfs);
    //BasicBlock *return_point = BasicBlock::Create(cont, "return_point", newfs);

    auto int_type_arg1 = fxpret1.scalarToLLVMType(cont);
    LLVM_DEBUG(dbgs() << "Argument fixed point type 1: ");
    int_type_arg1->print(dbgs(), true);
    LLVM_DEBUG(dbgs() << "\n");

    auto int_type_arg2 = fxpret2.scalarToLLVMType(cont);
    ...
    Value *internal_result = builder.CreateAlloca(int_type_ret, nullptr, "internal_result");


    {
    auto int_type = fxpret1.scalarToLLVMType(cont);
    ...
    if (!fxpret1.scalarIsSigned()) {
        ...
        fxpret1.scalarFracBitsAmt() = fxpret1.scalarFracBitsAmt() - 1;
        fxpret1.scalarIsSigned() = true;
    }
    ...
    */
    auto internal_fxpt = flttofix::FixedPointType(true, fxpret1.scalarFracBitsAmt() - 1, fxpret1.scalarBitsAmt());
    ...
    */
    ...
    builder.CreateStore(return_value, internal_result);
    builder.CreateBr(second_op);
    }

    builder.SetInsertPoint(second_op);

    Value* arg2 = builder.CreateLoad(int_type_arg2, arg2_ptr);
    Value *N = builder.CreateAlloca(int_type_ret, nullptr, "N");

    if (fxpret2.scalarFracBitsAmt() < fxpret.scalarFracBitsAmt()) {
        builder.CreateStore(builder.CreateShl(arg2, fxpret.scalarFracBitsAmt() - fxpret2.scalarFracBitsAmt()), N);
    }
    else
    {
        builder.CreateStore(builder.CreateAShr(arg2, fxpret2.scalarFracBitsAmt() - fxpret.scalarFracBitsAmt()), N);
    }

    Value* vero = builder.CreateICmpEQ(ConstantInt::get(int_type_ret,0), ConstantInt::get(int_type_ret, 0));

    std::string function_name("llvm.smul.fix.i");
    ...
    Value *exp_int = builder.CreateSelect(vero, N, N);

    LLVM_DEBUG(dbgs() << "AAAAAAAA\n");

    Value *mul = builder.CreateCall(umul, 
                {builder.CreateLoad(getElementTypeFromValuePointer(internal_result), internal_result), 
                builder.CreateLoad(int_type_ret, exp_int), 
                ConstantInt::get(fxpret.scalarToLLVMType(cont), fxpret.scalarFracBitsAmt())});

    builder.CreateStore(mul, internal_result);

    //builder.CreateStore(mul, result);
    //builder.CreateRet(builder.CreateLoad(getElementTypeFromValuePointer(internal_result), internal_result));




    
    builder.CreateBr(third_op);

    {
    builder.SetInsertPoint(third_op);
    ...
    // Generate strings for constants names
    std::string S_arg_point = "." + std::to_string(fxpret.scalarFracBitsAmt());
    std::string S_ret_point = "." + std::to_string(fxpret.scalarFracBitsAmt());
    std::string S_int_point = "." + std::to_string(internal_fxpt.scalarFracBitsAmt());
    ...
    builder.CreateStore(mul, arg_ptr);
    ...
    */
    if (fxpret.scalarIntegerBitsAmt() < 3) {
        LLVM_DEBUG(dbgs() << "Argument is too small: shifting argument to the right by " << 3 - fxpret.scalarIntegerBitsAmt() << " bit(s)\n");
        builder.CreateStore(builder.CreateAShr(builder.CreateLoad(getElementTypeFromValuePointer(arg_ptr), arg_ptr), ConstantInt::get(int_type_narrow, 3 - fxpret.scalarIntegerBitsAmt()), "arg_shifted_was_too_small"), arg_ptr);
        fxpret.scalarFracBitsAmt() = fxpret.scalarBitsAmt() - 3;
        LLVM_DEBUG(dbgs() << "New argument fixed point type: " << fxpret << "\n");
    }
    ...
    TaffoMath::pair_ftp_value<llvm::Constant *> one_ptr_arg(fxpret);
    bool one_ptr_arg_created = TaffoMath::createFixedPointFromConst(
        cont, ref, 1.0, fxpret, one_ptr_arg.value, one_ptr_arg.fpt);
    ...
    TaffoMath::pair_ftp_value<llvm::Constant *> minus_one_ptr_arg(fxpret);
    bool minus_one_ptr_arg_created = TaffoMath::createFixedPointFromConst(
        cont, ref, -1.0, fxpret, minus_one_ptr_arg.value, minus_one_ptr_arg.fpt);
    ...
    */
    ...
    // Pointer to An in the internal (wide) fixed point type
    TaffoMath::pair_ftp_value<llvm::Constant *> ln2_ptr(fxpret);
    ...
    bool ln2_ptr_created = TaffoMath::createFixedPointFromConst(
        cont, ref, flttofix::ln2, fxpret, ln2_ptr.value, ln2_ptr.fpt);
    ...
    */
                        ...
                        check_loop_positives);
    {
        ...
        Value *arg_value_wide = builder.CreateShl(builder.CreateSExt(arg_value_narrow, int_type_wide, "sign_extended_arg_loop1"), internal_fxpt.scalarFracBitsAmt() - fxpret.scalarFracBitsAmt(), "curr_arg_val_wide_loop1");
                        ...
                        finalize);
    {
        ...
        Value *arg_value_wide = builder.CreateShl(builder.CreateSExt(arg_value_narrow, int_type_wide, "sign_extended_arg_loop2"), internal_fxpt.scalarFracBitsAmt() - fxpret.scalarFracBitsAmt(), "curr_arg_val_wide_loop2");
        // Current x and y values
        auto x_value = builder.CreateLoad(getElementTypeFromValuePointer(x_ptr.value), x_ptr.value, "x_curr_loop2");
        auto y_value = builder.CreateLoad(getElementTypeFromValuePointer(y_ptr.value), y_ptr.value, "y_curr_loop2");
    ...
    builder.CreateStore(builder.CreateShl(output_temp, 1, "output_temp_shifted"), return_value_ptr);
