...
bool createTan(FloatToFixed *ref,
               llvm::Function *newfs, llvm::Function *oldf)
{
  //
  newfs->deleteBody();
  ...
  auto specialAngle =
      builder.CreateAlloca(Type::getInt8Ty(cont), nullptr, "specialAngle");
  Value *arg_value = builder.CreateAlloca(int_type, nullptr, "arg");
  Value *cos_value = builder.CreateAlloca(int_type, nullptr, "cosx");
  Value *sin_value = builder.CreateAlloca(int_type, nullptr, "sinx");
  Value *tan_value = builder.CreateAlloca(int_type, nullptr, "tanx");
  builder.CreateStore(newfs->getArg(0), arg_value);
  Value *i_iterator = builder.CreateAlloca(int_type, nullptr, "iterator");
...

  {
    ...
    builder.CreateStore(builder.CreateSelect(
                            builder.CreateICmpEQ(builder.CreateLoad(getElementTypeFromValuePointer(changeSign), changeSign), zero_bool),
                            builder.CreateLoad(getElementTypeFromValuePointer(sin_value), sin_value),
                            builder.CreateSub(zero_arg, builder.CreateLoad(getElementTypeFromValuePointer(sin_value), sin_value))),
                        sin_value);
  }

  {
    std::string function_name("llvm.sdiv.fix.i");
    function_name.append(std::to_string(truefxpret.scalarToLLVMType(cont)->getScalarSizeInBits()));

    Function *sdiv = M->getFunction(function_name);
    if (sdiv == nullptr) {
      std::vector<llvm::Type *> fun_arguments;
      fun_arguments.push_back(
          truefxpret.scalarToLLVMType(cont)); // depends on your type
      fun_arguments.push_back(
          truefxpret.scalarToLLVMType(cont)); // depends on your type
      fun_arguments.push_back(Type::getInt32Ty(cont));
      FunctionType *fun_type = FunctionType::get(
          truefxpret.scalarToLLVMType(cont), fun_arguments, false);
      sdiv = llvm::Function::Create(fun_type, GlobalValue::ExternalLinkage,
                                    function_name, M);
    }


    LLVM_DEBUG(dbgs() << "internal_fxpt: " << internal_fxpt.scalarFracBitsAmt() << " truefxpret: " << truefxpret.scalarFracBitsAmt() << "\n");

    if (internal_fxpt.scalarFracBitsAmt() > truefxpret.scalarFracBitsAmt()) {
      auto sin = builder.CreateAShr(builder.CreateLoad(getElementTypeFromValuePointer(sin_value), sin_value), internal_fxpt.scalarFracBitsAmt() - truefxpret.scalarFracBitsAmt());
      auto cos = builder.CreateAShr(builder.CreateLoad(getElementTypeFromValuePointer(cos_value), cos_value), internal_fxpt.scalarFracBitsAmt() - truefxpret.scalarFracBitsAmt());
      builder.CreateStore(sin, sin_value);
      builder.CreateStore(cos, cos_value);
    } else if (internal_fxpt.scalarFracBitsAmt() < truefxpret.scalarFracBitsAmt()) {
      auto sin = builder.CreateShl(builder.CreateLoad(getElementTypeFromValuePointer(sin_value), sin_value), truefxpret.scalarFracBitsAmt() - internal_fxpt.scalarFracBitsAmt());
      auto cos = builder.CreateShl(builder.CreateLoad(getElementTypeFromValuePointer(cos_value), cos_value), truefxpret.scalarFracBitsAmt() - internal_fxpt.scalarFracBitsAmt());
      builder.CreateStore(sin, sin_value);
      builder.CreateStore(cos, cos_value);
    }

    auto tan = builder.CreateCall(
        sdiv, {builder.CreateLoad(getElementTypeFromValuePointer(sin_value), sin_value), builder.CreateLoad(getElementTypeFromValuePointer(cos_value), cos_value),
               llvm::ConstantInt::get(truefxpret.scalarToLLVMType(cont),
                                      truefxpret.scalarFracBitsAmt())});
    builder.CreateStore(tan, tan_value);
  }

  LLVM_DEBUG(dbgs() << "tan_value: ");
  tan_value->print(dbgs(), true);
  LLVM_DEBUG(dbgs() << "\n");

  auto ret = builder.CreateLoad(getElementTypeFromValuePointer(tan_value), tan_value);
