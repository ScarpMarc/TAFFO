...
bool createCbrt(FloatToFixed *ref, llvm::Function *newfs, llvm::Function *oldf)
{
    ...
    TaffoMath::getFixedFromArg(ref, oldf, fxpret1, 0, foundArg);
    ...
    // Pointer to 3 in the return (narrow) fixed point type
    TaffoMath::pair_ftp_value<llvm::Constant *> three_ptr(fxpret);

    bool three_ptr_created = TaffoMath::createFixedPointFromConst(
        cont, ref, 3.0, fxpret, three_ptr.value, three_ptr.fpt);
    
    if (!three_ptr_created) {
        LLVM_DEBUG(dbgs() << "Could not create three_ptr\n");
        return false;
    }

    three_ptr.value = TaffoMath::createGlobalConst(
        M, "three_ptr" + S_ret_point, three_ptr.fpt.scalarToLLVMType(cont), three_ptr.value,
        dataLayout.getPrefTypeAlignment(three_ptr.fpt.scalarToLLVMType(cont)));
    ...
    */
    ...
    std::string function_name("llvm.sdiv.fix.i");
    ...
    Value *div = builder.CreateCall(udiv, 
                {builder.CreateLoad(getElementTypeFromValuePointer(internal_result), internal_result), 
                builder.CreateLoad(getElementTypeFromValuePointer(three_ptr.value), three_ptr.value), 
                ConstantInt::get(fxpret.scalarToLLVMType(cont), fxpret.scalarFracBitsAmt())});

    builder.CreateStore(div, internal_result);
    ...
    builder.CreateStore(div, arg_ptr);
